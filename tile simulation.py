...
class SiloSystem:
    ...
    def feed_from_dryer(self, env, tile_id, weight):# weight is the weight of the tile being fed into the silo, which corresponds to the amount of powder being added to the silo after drying
        """Dryer adds dried material into the emptier silo."""
        idx = 0 if self.levels[0] <= self.levels[1] else 1#feed the dryer output into the emptier silo to keep them balanced
    ...
    def consume_for_press(self, env, tile_id, weight):
        """Press consumes material from the fuller silo."""
        idx = 0 if self.levels[0] >= self.levels[1] else 1

        if self.levels[idx] >= weight:
            self.levels[idx] -= weight
            print(f"{env.now:8.1f}  Tile {tile_id} PRESS consumes {weight} kg from Silo {idx} "
                  f"→ levels: {self.levels[0]:.1f} kg, {self.levels[1]:.1f} kg")
        else:
            print(f"{env.now:8.1f}  Tile {tile_id} PRESS wants {weight} kg from Silo {idx} "
